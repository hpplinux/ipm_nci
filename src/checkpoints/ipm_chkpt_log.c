void ipm_chkpt_log(void) { /* called by all tasks (or as many as possible) */
    int i,ii,rv,icall,ibytes,irank,ireg,kreg;
    int ipm_log_fd=-1, fan_out=0;
    int log_rank=-1, search_offset=0, token=1;
    FILE *ipm_mpi_chkpt_fh;
    MPI_File ipm_mpiio_chkpt_fh;
    DIR  *ipm_mpi_log_dp;
    struct dirent *de;
    ...
    char chkpt_fname[MAXSIZE_FILENAME];    
...
//    task.flags |= IPM_WROTELOG;
    ...
    if(task.flags & DEBUG) {
	printf("IPM: %d checkpoint log enter job.cookie=%s username=%s \n",
	       task.mpi_rank,
	       job.cookie,
...
*/
	...
	if(task.flags & DEBUG) {
	    printf("IPM: %d checkpoint log exit due to LOGDIR=/dev/null", task.mpi_rank);
	}
	return;
    }
...
#define IPM_LOG_USEMULTI
    ...
    {
	sprintf(tmp_fname,"%s/%s.%s",job.log_dir,job.log_fname,"chkpt");
	sprintf(chkpt_fname,"%s",tmp_fname);
    }
#endif
    
    if(task.flags & DEBUG) {
	printf("IPM: %d checkpoint log IPMLOG_DIR=%s FNAME=%s \n",
	       task.mpi_rank, 
	       job.log_dir, 
	       chkpt_fname);
    }
...
/* simplest case no locking just write each file. Parallel FS may 
   have metadata storm for N file creates */
    ...
    if(task.flags & DEBUG) {
	printf("IPM: %d checkpoint log using IPM_LOG_USEMULTI. \n",
	       task.mpi_rank);
	fflush(stdout);
    }

    
    ipm_mpi_chkpt_fh = fopen(chkpt_fname,"w");
    if(ipm_mpi_chkpt_fh == NULL) {
	printf("IPM: %d log fopen failed fname=%s \n",
	       task.mpi_rank, 
	       chkpt_fname); fflush(stdout);
    }
    rv = fprintf(ipm_mpi_chkpt_fh,
		 "<?xml version=\"1.0\" encoding=\"iso-8859-1\"?>\n<ipm>\n"); 
    ipm_log_write(&job, &task, txt, ipm_mpi_chkpt_fh, 0, 0);
    rv = fprintf(ipm_mpi_chkpt_fh,
		 "</ipm>\n"); 
    fclose(ipm_mpi_chkpt_fh);
    chmod(chkpt_fname, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH);

#undef IPM_LOG_USEMULTI    
    return;
#endif
/* } */
...
*/
    ...
    if(task.flags & DEBUG) {
	printf("IPM: %d checkpoint log using IPM_LOG_USELOCKS. \n",
	       task.mpi_rank);
	fflush(stdout);
    }
    ...
    if(task.flags & DEBUG) {
	printf("IPM: %d checkpoint log block_lock fname=%s fd=%d stamp=%f \n",
	       task.mpi_rank, 
	       chkpt_fname,
	       ipm_log_fd,
	       stamp1
    ...
    IPM_FILE_LOCK(chkpt_fname,ipm_log_fd);
    
    IPM_TIME_GTOD(stamp2);
    if(task.flags & DEBUG) {
	printf("IPM: %d checkpoint log block_lock fname=%s fd=%d stamp=%12.6f delta=%.3e \n",
	       task.mpi_rank, 
	       chkpt_fname,
	       ipm_log_fd,
    ...
    ipm_mpi_chkpt_fh = fdopen(ipm_log_fd,"w+");
    
    if(!ipm_mpi_chkpt_fh || !ipm_log_fd) {
/* fail silently */
	return;
    }
    
/* got log fh */
    fseek(ipm_mpi_chkpt_fh,0,SEEK_END);
    ipm_log_write_task(&job, &task, txt, ipm_mpi_chkpt_fh);
    
    IPM_TIME_GTOD(stamp3);
    if(task.flags & DEBUG) {
	printf("IPM: %d checkpoint log write fname=%s fd=%d stamp=%12.6f delta=%.3e \n",
	       task.mpi_rank, 
	       chkpt_fname,
	       ipm_log_fd,
    ...
    fflush(ipm_mpi_chkpt_fh);
    IPM_FILE_UNLOCK(chkpt_fname,ipm_log_fd);
    
    IPM_TIME_GTOD(stamp4);
    if(task.flags & DEBUG) {
	printf("IPM: %d checkpoint log unlock fname=%s fd=%d stamp=%12.6f delta=%.3e \n",
	       task.mpi_rank, 
	       chkpt_fname,
	       ipm_log_fd,
...
*/
    ...
    if(task.flags & DEBUG) {
	printf("IPM: %d checkpoint log using IPM_LOG_USESMPFS. \n",
	       task.mpi_rank);
	fflush(stdout);
    }
	...
	sprintf(tmp_fname,"/tmp/%s.%s.%d",
		...
		job.cookie,
		task.mpi_rank);
	ipm_mpi_chkpt_fh = fopen(tmp_fname,"w");
	if(ipm_mpi_chkpt_fh == NULL) {
	    printf("IPM: %d log fopen failed fname=%s \n",
		   task.mpi_rank, 
...
*/
    ...
    if(task.flags & DEBUG) {
	printf("IPM: %d checkpoint log using IPM_LOG_USEMPI. \n",
	       task.mpi_rank);
	fflush(stdout);
    }
...
/* measure size of buff required */
	...
	if (malloc_sum == task.mpi_size)  {/* use parallel IO */
		...
		printf("IPM: %d checkpoint IPM report parallel IO used\n", task.mpi_rank);
	    }
...
/* so we just delete so that we don't end up with trailing garbage  */
	    if (!task.mpi_rank) rv=PMPI_File_delete ( chkpt_fname,MPI_INFO_NULL);
	    rv=PMPI_Barrier(MPI_COMM_WORLD);
	    
	    rv = PMPI_File_open( MPI_COMM_WORLD, chkpt_fname, MPI_MODE_WRONLY | MPI_MODE_CREATE,info,  &ipm_mpiio_chkpt_fh );
	    if (rv)
	    {
...
#define MPI_INTEGER8 MPI_LONG_LONG_INT
	    ...
	    rv=PMPI_File_set_view(ipm_mpiio_chkpt_fh,file_offset,MPI_CHAR, MPI_CHAR,"native",info);
	    
/*write info*/
	    rv=PMPI_File_write_all(ipm_mpiio_chkpt_fh,buffer,buff_size,MPI_CHAR,MPI_STATUS_IGNORE);
	    
	    
	    rv = PMPI_File_close( &ipm_mpiio_chkpt_fh );
	    PMPI_Barrier(MPI_COMM_WORLD);
/* Some MPI-IO implimentations (cray) permissions are not setable with hints */
	    if (task.mpi_rank == 0) chmod (chkpt_fname,0744);
	    free (buffer); 
	    return;
	} else {
