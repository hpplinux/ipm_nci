...
int bsize;
int ipmprintf (void* fh,size_t buff_in,int flag,const char* format,...){
  int rv;
  va_list ap;
  va_start (ap,format);
  if (flag ==0){
    rv=vfprintf((FILE*)fh,format,ap);
      }
  else{
    rv=vsnprintf((char*)((char*)fh+bsize),buff_in,format,ap);
    bsize+=rv;
      }
  va_end(ap);
  return rv;
 ...
 int ipm_log_write(struct ipm_jobdata *j, 
		   struct ipm_taskdata *t, 
		   char *literal, 
		   void *fh,
		   size_t buff_in,
		   int pflag) { 
...
/*********************************************/
  if(t->mpi_rank==0) {
   ...
   rv = ipmprintf(fh,buff_in,pflag, "<task ipm_version=\"%s\" cookie=\"%s\" mpi_rank=\"%d\" mpi_size=\"%d\" stamp_init=\"%.6f\" stamp_final=\"%.6f\" username=\"%s\" groupname=\"%s\" flags=\"%lld\" pid=\"%d\" >\n",
	IPM_VERSION,
	j->cookie,
	t->mpi_rank,
	t->mpi_size,
...
#define JIE_HACK_EXECINFO
   ...
   if(task.mpi_rank == 0)
       fp1 = fopen(exe_file, "r");
   
   if(fp1 && task.mpi_rank == 0) { 
       char tmp[MAXSIZE_TXTLINE];
       while(fgets(tmp, MAXSIZE_TXTLINE, fp1)) {
	   rv=ipmprintf(fh, buff_in, pflag, "%s", tmp);
       }
       fclose(fp1);
   } else {
       if(task.mpi_rank == 0) {
	   printf("in ipm_log_write(), file: %s -- Open failed.\n", exe_file);
	   fflush(stdout);
       }
   ...
   if(task.mpi_rank == 0) 
       fp2 = fopen(ldd_file, "r");

   if(fp2 && task.mpi_rank == 0) {       
       char tmp[MAXSIZE_TXTLINE];
       while(fgets(tmp, MAXSIZE_TXTLINE, fp2)) {
	   rv=ipmprintf(fh, buff_in, pflag, "%s", tmp);
       }
       fclose(fp2);      
   } else {
